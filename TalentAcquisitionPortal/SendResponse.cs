...
namespace TalentAcquisitionPortal
{
    public class SendResponse
    {
        ...
        public void SendEmail_HODTAB(string mBody, string sub)
        {
            try
            {
                DataTable dt = new DataTable();
                string ToID = "automail.hr@alkaram.com";
                using (MailMessage message = new MailMessage("mis@alkaram.com", ToID))
                {
                    message.Subject = sub;
                    message.Body = mBody;
                    ...
                    if (CCdt.Rows.Count > 0)
                    {
                        ...
                        foreach (string email in ccEmails.ToString().Split(','))
                        {
        ...
        public void SendEmailTA2(string ToID, string mBody, string sub)
        {
            try
            {
                //string From = "mis@alkaram.com";
                string To = ToID;
                ToID = "muhammad.mubbashir@alkaram.com";
                string Subject = sub;
